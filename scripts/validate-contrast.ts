 ...
 *
 * Usage:
 *   node scripts/validate-contrast.ts <theme-file>
 *
 * Exit codes:
 ...
 */
function resolveColor(theme: Theme, colorRef: string, mode: 'dark' | 'light'): string | null {
  ...
  if (category && colorMode && theme.theme[category]) {
    const ref = theme.theme[category] as { dark: string; light: string };
    if (ref[mode]) {
 ...
 */
function getRecommendation(pair: ColorPair): string {
 ...
 */
function printReport(themeFile: string, allPairs: ColorPair[]): void {
...
function main(): void {
  ...
  if (args.length === 0) {
    ...
    console.error('Usage: node scripts/validate-contrast.ts <theme-file>');
    process.exit(2);
  }
  ...
  for (const pair of pairs) {
    ...
    const fgRef = pair.fg.replace('.dark', '').replace('.light', '');
    const bgRef = pair.bg.replace('.dark', '').replace('.light', '');
